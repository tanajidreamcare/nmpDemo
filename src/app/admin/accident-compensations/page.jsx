...
function Page() {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete('/admin/accident-compensation/delete/' + id);
  ...
  const handleEdit = (id) => {
    router.push(`/admin/accident-compensations/edit/${id}`);
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) =>
        row.filename ? (
          <a href={`http://3.111.197.119/api/uploads/${row.filename}`} target="_blank" download={row.name}>
  ...
  return (
    <div className="mt-16 ml-6">
      <div className="h-screen w-full mr-6 ">
        <h1 className="w-full text-center font-extrabold text-3xl my-5 ">Accident Compensation</h1>
        <div className="mt-10 flex justify-center">
          <button
            onClick={() => router.push('/admin/accident-compensations/add')}
            type="submit"
